...
const app = {
  setup() {
    ...
    const changeAlbum = (idx) => {
      ...
      // console.log(works[currentIdx.value]);
      setTimeout(() => {
    ...
    const getImgUrl = (idx) => {
      return idx < 9
        ? `${api}${works[currentIdx.value].albumFolder}00${idx + 1}.jpg`
        : `${api}${works[currentIdx.value].albumFolder}0${idx + 1}.jpg`;
    };
    ...
    onMounted(() => {
      console.clear();
      currentIdx.value = getRandomNum(0, works.length);
